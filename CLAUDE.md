...
- Config release work currently uses `config_version: 22`. Schema changes must update bundled defaults, docs, release checks, and migration behavior together. v21 adds `smart_tab_mode`, introduces the optional `SmartPrompt` value for `window_close_confirmation` (default stays `NeverPrompt`), and accepts the removed `language` option as a deprecated field for backward compat. v22 adds a precmd guard so the dark-theme comment color override still applies when the user pre-loads fast-syntax-highlighting or zsh-syntax-highlighting in their own `.zshrc`.
